...
import fallproject.FinderApp;
import fallproject.Home;
import fallproject.HomeAmenities;
import fallproject.HomeMaterial;
import fallproject.HomePets;
import fallproject.HomePlacement;
import fallproject.HomeType;
import fallproject.MockEmailHolder;
...
//dependency Injection
	...
	public void aListOfHomesThatMatchesTheTypeSpecificationShouldBeReturnedAndPrintedOnTheConsole() {
		...
		if(spec.equals("HOUSE")) {
			...
			while(itr.hasNext()){
				Assert.assertEquals(itr.next().gettype().toString(),spec);
				
			}	
			System.out.print(byTypeList.toString());
...
public void aListOfHomesThatMatchesTheLeaseLengthSpecificationShouldBeReturnedAndPrintedOnTheConsole() {
	...
	while(itr.hasNext()){
		Home h= itr.next();
		assertTrue(h.getleaselength() <= lease);}
	...
	public void aListOfHomesThatMatchesThePriceSpecificationShouldBeReturnedAndPrintedOnTheConsole() {
	   ...
	   // throw new io.cucumber.java.PendingException();
		...
		System.out.print(byPriceList.toString());
	}
   ...
   // throw new io.cucumber.java.PendingException();
	...
	if(spec.equals("WOOD")) {
		...
		while(itr.hasNext()) {
			...
			Assert.assertEquals(itr.next().getmaterial().toString(),spec);
		}
		System.out.print(byMaterialList.toString());
	}
...
public void aListOfHomesThatMatchesThePlacementSpecificationShouldBeReturnedAndPrintedOnTheConsole() {
	...
	if(spec.equals("CITY")) {
		...
		while(itr.hasNext()) {
			...
			Assert.assertEquals(itr.next().getPlacement().toString(),spec);
		}
	}
	...
	}System.out.print(byPlacementList.toString());
...
public void aListOfHomesThatMatchesTheAmenitiesSpecificationShouldBeReturnedAndPrintedOnTheConsole() {
	...
	if(spec.equals("AIRCONDITIONING")) {
		...
		while(itr.hasNext()) {
			...
			Assert.assertEquals(itr.next().getamenities().toString(),spec);
	...
	while(itr.hasNext()){
		Assert.assertEquals(itr.next().getamenities().toString(),spec);
	}System.out.print( byAmenityList.toString());
...
public void aListOfHomesThatMatchesTheAreaSpecificationShouldBeReturnedAndPrintedOnTheConsole() {
   ...
   // throw new io.cucumber.java.PendingException();
	...
	}	System.out.print(byAreaList.toString());
}
...
public void aListOfHomesThatMatchesTheNumberOfBedroomsSpecificationShouldBeReturnedAndPrintedOnTheConsole() {
	...
	//assertEquals("found 2 homes ", 2, ((List<Home>) byBedList).size());
		...
		while(itr.hasNext()){
			Home h= itr.next();
			assertTrue(h.getnumberofbathrooms() < bed||h.getnumberofbathrooms() == bed);
		}System.out.print( byBedList.toString());
...
public void iSearchAboutHomeByPets(String pet) {
    // Write code here that turns the phrase above into concrete actions
   // throw new io.cucumber.java.PendingException();
	this.pet= spec;
	this.spec=pet;
   ...
   // throw new io.cucumber.java.PendingException();
	...
	if(pet.equals("Yes")) {
		...
		while(itr.hasNext()) {
			...
			Assert.assertEquals(itr.next().getpets().toString(),pet);
	...
	}System.out.print( byPetsList.toString());
}
...
public void aListOfHomesThatMatchesTheNumberOfBathroomsSpecificationShouldBeReturnedAndPrintedOnTheConsole() {
	...
	while(itr.hasNext()){
		Home h= itr.next();
		assertTrue(h.getnumberofbathrooms() < bath||h.getnumberofbathrooms() ==bath);
		
	}System.out.print( byBathList.toString());
	
}

@Then ("email with the result should be send to user {string}")
public void emailWithTheResultShouldBeSendToUser(String email1) {
	verify(mockWebholder.getEmailService(),times(1)).sendEmail(email1,FinderApp.MaterialList);
}
